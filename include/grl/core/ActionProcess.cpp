...
ActionProcess::ActionProcess(TriggerHandler* th_, std::string name_)
{
	...
	th->register_action_process(grl::utils::reverse_hash(name_), this);
	asset_name = name_;
}
...
{
	printf("Im %s and I sent a message to %s\n", asset_name.c_str(), hash.c_str());
	notify_trigger_handler(grl::utils::reverse_hash(hash));
}

void ActionProcess::trigger_handler_callback(grl::utils::trigger_message tm)
{
	printf("objective %s, I recieved %d %d %d\n", asset_name.c_str(), tm.flag, tm.dest, tm.data);
}

void ActionProcess::notify_trigger_handler(grl::utils::trigger_message tm)
{
