...
class State:
    ...
    def associate_transition_function(self, func_desc, state):
        if func_desc not in self.transition_functions:
            ...
            return
        self.transition_functions[func_desc][0] = state
    ...
    def add_transition_function(self, description, state = None, function):
        self.transition_functions[description] = (state, function)
    ...
    def poll(self):
        ...
        for i in self.transition_functions:
            ret = self.transition_functions[i][1]()
            if ret:
                triggered_transition_funcs.append(self.transition_functions[i][0])
                move_on = True
        if move_on:
...
class StateMachine:
    ...
    def __init__(self, starting_state: State, alphabet_dict: dict, states, state_description = "A State Machine"):
        ...
        for i in states:
            self.add_state(i)
        if starting_state not int self.states:
            self.invalid_state(starting_state)
        else:
            self.current_state = starting_state
            self.staring_state = staring_state
