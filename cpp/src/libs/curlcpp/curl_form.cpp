...
// Implementation of assignment operator. Also here, we must perform a deep copy
// of the entire list.
curl_form &curl_form::operator=(const curl_form &form) {
  ...
  while (old_head != nullptr) {
    if (this->form_post == nullptr) {
      this->is_null(
          this->last_ptr = this->form_post =
              (struct curl_httppost *)malloc(sizeof(struct curl_httppost)));
      this->copy_ptr(&this->last_ptr, old_head);
    } else {
      this->is_null(this->last_ptr->next = (struct curl_httppost *)malloc(
                        sizeof(struct curl_httppost)));
...
void curl_form::add(const curl_pair<CURLformoption, string> &form_name,
  ...
  if (curl_formadd(&this->form_post, &this->last_ptr, form_name.first(),
                   form_name.second(), form_content.first(),
                   form_content.second(), CURLFORM_END) != 0) {
...
void curl_form::add(const curl_pair<CURLformoption, string> &form_name,
  ...
  if (curl_formadd(&this->form_post, &this->last_ptr, form_name.first(),
                   form_name.second(), form_content.first(),
                   form_content.second(), content_type.first(),
                   content_type.second(), CURLFORM_END) != 0) {
...
void curl_form::add(const curl_pair<CURLformoption, string> &form_name,
  ...
  if (curl_formadd(&this->form_post, &this->last_ptr, form_name.first(),
                   form_name.second(), form_content.first(),
                   form_content.second(), content_length.first(),
                   content_length.second(), CURLFORM_END) != 0) {
...
void curl_form::add(const curl_pair<CURLformoption, string> &form_name,
  ...
  if (curl_formadd(&this->form_post, &this->last_ptr, form_name.first(),
                   form_name.second(), form_content.first(),
                   form_content.second(), name_length.first(),
                   name_length.second(), CURLFORM_END) != 0) {
...
void curl_form::add(const curl_pair<CURLformoption, string> &form_name,
  ...
  if (curl_formadd(&this->form_post, &this->last_ptr, form_name.first(),
                   form_name.second(), form_content.first(),
                   form_content.second(), content_length.first(),
                   content_length.second(), content_type.first(),
                   content_type.second(), CURLFORM_END) != 0) {
...
void curl_form::add(const curl_pair<CURLformoption, string> &form_name,
  ...
  if (curl_formadd(&this->form_post, &this->last_ptr, form_name.first(),
                   form_name.second(), form_bufname.first(),
                   form_bufname.second(), form_content.first(),
                   form_content.second(), content_length.first(),
                   content_length.second(), CURLFORM_END) != 0) {
...
void curl_form::add(const curl_pair<CURLformoption, string> &form_name,
  ...
  if (curl_formadd(&this->form_post, &this->last_ptr, form_name.first(),
                   form_name.second(), CURLFORM_ARRAY, new_files,
                   CURLFORM_END) != 0) {
    delete[] new_files;
...
/**
 * Helper function used to copy the curl_httppost list in the copy constructor,
 * to reduce code verbosity. We must allocate a pointer for every single pointer
 * data in the old list to perform a complete deep copy.
 */
void curl_form::copy_ptr(struct curl_httppost **ptr,
                         const struct curl_httppost *old_ptr) {
  if (old_ptr->name) {
    this->is_null((*ptr)->name =
                      (char *)malloc(old_ptr->namelength * sizeof(char)));
  ...
  if (old_ptr->buffer) {
    // Copy the buffers
    this->is_null((*ptr)->buffer =
                      (char *)malloc(old_ptr->bufferlength * sizeof(char)));
  ...
  if (old_ptr->contents) {
    // Copy the contents
    this->is_null((*ptr)->contents =
                      (char *)malloc(old_ptr->contentslength * sizeof(char)));
  ...
  if (old_ptr->contenttype) {
    // Copy content type
    this->is_null((*ptr)->contenttype = (char *)malloc(
                      strlen(old_ptr->contenttype) * sizeof(char)));
    strcpy((*ptr)->contenttype, old_ptr->contenttype);
  }
  if (old_ptr->showfilename) {
    // Copy file name
    this->is_null((*ptr)->showfilename = (char *)malloc(
                      strlen(old_ptr->showfilename) * sizeof(char)));
