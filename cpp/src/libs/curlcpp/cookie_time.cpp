...
curl::cookie_time::cookie_time(const unsigned int hour,
...
curl::cookie_time *
curl::cookie_time::set_minutes(unsigned int _minutes) NOEXCEPT {
...
curl::cookie_time *
curl::cookie_time::set_seconds(unsigned int _seconds) NOEXCEPT {
...
std::string curl::cookie_time::get_formatted() NOEXCEPT {
  ...
  stream << this->get_hour() << ":" << this->get_minutes() << ":"
         << this->get_seconds() << " GMT";
  return stream.str();
}
