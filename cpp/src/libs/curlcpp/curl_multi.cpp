...
curl_multi::curl_multi(const long flag)
    : curl_interface(flag), curl(curl_multi_init()) {
...
void curl_multi::add(const curl_easy &easy) {
  const CURLMcode code =
      curl_multi_add_handle(this->curl.get(), easy.get_curl());
  if (code == CURLM_OK) {
...
void curl_multi::remove(const curl_easy &easy) {
  const CURLMcode code =
      curl_multi_remove_handle(this->curl.get(), easy.get_curl());
  if (code == CURLM_OK) {
...
vector<unique_ptr<curl_multi::curl_message>> curl_multi::get_info() {
  ...
  while ((message =
              curl_multi_info_read(this->curl.get(), &this->message_queued))) {
    infos.push_back(unique_ptr<curl_multi::curl_message>(
        new curl_multi::curl_message(message)));
  }
  return infos;
}

// Implementation of overloaded get_info method.
unique_ptr<curl_multi::curl_message>
curl_multi::get_info(const curl_easy &easy) {
  CURLMsg *message = nullptr;
  while ((message =
              curl_multi_info_read(this->curl.get(), &this->message_queued))) {
    if (message->easy_handle == easy.get_curl()) {
      unique_ptr<curl_multi::curl_message> ptr{
          new curl_multi::curl_message(message)};
      return ptr;
    }
  }
...
curl_easy *curl_multi::get_next_finished() {
  ...
  if (message->msg == CURLMSG_DONE) {
    std::unordered_map<CURL *, curl_easy *>::const_iterator it =
        handles.find(message->easy_handle);
    if (it != handles.end()) {
      return it->second;
    }
  }
...
bool curl_multi::is_finished(const curl_easy &easy) {
  ...
  while ((message =
              curl_multi_info_read(this->curl.get(), &this->message_queued))) {
    if (message->easy_handle == easy.get_curl() and
...
bool curl_multi::perform() {
  const CURLMcode code =
      curl_multi_perform(this->curl.get(), &this->active_transfers);
...
bool curl_multi::socket_action(const curl_socket_t sockfd,
                               const int ev_bitmask) {
  const CURLMcode code = curl_multi_socket_action(
      this->curl.get(), sockfd, ev_bitmask, &this->active_transfers);
  if (code == CURLM_CALL_MULTI_PERFORM) {
    return false;
  }
  if (code != CURLM_OK) {
...
void curl_multi::set_descriptors(fd_set *read, fd_set *write, fd_set *exec,
                                 int *max_fd) {
  const CURLMcode code =
      curl_multi_fdset(this->curl.get(), read, write, exec, max_fd);
  if (code != CURLM_OK) {
    throw curl_multi_exception(code, __FUNCTION__);
  }
}
...
void curl_multi::wait(struct curl_waitfd extra_fds[],
                      const unsigned int extra_nfds, const int timeout_ms,
                      int *numfds) {
  const CURLMcode code = curl_multi_wait(this->curl.get(), extra_fds,
                                         extra_nfds, timeout_ms, numfds);
  if (code != CURLM_OK) {
    throw curl_multi_exception(code, __FUNCTION__);
  }
}
...
CURLM *curl_multi::get_curl() const { return this->curl.get(); }
