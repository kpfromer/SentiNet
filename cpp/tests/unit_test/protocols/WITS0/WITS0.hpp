...
#include "protocols/WITS0/WITS0.h"
#include "common/descriptor/Fd.hpp"

// The custom GTEST harness for the WITS0 testing
class WITS0Test : public GtestHarness {
 public:
  void SetUp() {
    ::GtestHarness::SetUp();
 ...
 */
class WITS0Application : public WITS0::WITS0 {
 public:
  WITS0Application() {
    // @todo [KEMB-556] Saving all the unique pointers from the register() methods will eventually need to be done
    // Bind on the registration method so that we can capture the std::unique pointer and keep it in scope
    WITS0::WITS0::bind<ApplicationInterface::BoundDescriptorFunc>(
  ...
  bool initialize(const ConfigurationInterface& config) {
    ...
    return ret && WITS0::WITS0::initialize(config);
  }
  ...
  void set_name(const std::string& name) {
    WITS0::WITS0::set_name(name);
  }

  void process_data(DescriptorInterface* descriptor) {
    WITS0::WITS0::process_data(descriptor);
  }

  bool send_handshake() {
    return WITS0::WITS0::send_handshake();
  }

  void bind_publish_callback(PublishFunc callback) {
    WITS0::WITS0::bind_publish_callback(callback);
  }
