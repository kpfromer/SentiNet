...
inline float DC_Motor::convert_throttle(){
  return (pwm_zero_throttle + (constrain(throttle, -maxThrottle, maxThrottle) * scalar));
}

DC_Motor::DC_Motor(const ConfigurationClient& config, const std::string& name, const std::string& name_)
...
void emergency_cut_power(){analogWriteFrequency(pin, pwm_zero_throttle);
