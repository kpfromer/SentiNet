 ...
 *
 * LINKER - action_objective - this is an arbitrary pointer to a pool of data that is mutated by 
 * ControlClientInterface and is read only for Robot
 *
 */
//defaults
class RobotInterface : public RobotInterfaceInterface
{
  ...
  private:
    //Instead of initializing constants at runtime, constants are a pointer so a user can
    //point their robot system to a set of commands - ie linear angular movement controls,
    //this way a user can alter the data during runtime
