...
class ServerInterface
{
    public:
		 ...
		 *
		 * This constructor simply sets serving address to address and threads to 1
		 * it is here to be able to interface with other implimented server types
		 *
		 ...
		 * strong effect on the opperating system. This constructor puts an amount of
		 * worker threads that all listen to the primary thread. Note that worker threads
		 * are initialized on the stack in the method multi_threaded_listener.
		 *
		 ...
		 * an int should capture as much information as we need for now. If needed,
		 * there can be meta information stored in the bits of the int
		 *
		 * @param context A number that may or may not need to be passed through the method. 
		 *
		 ...
		 *
		 * This is very much up to the implimentation. This can be infinitely looped or
		 * not. ZMQ for example has ports tied to processes. I tried splitting this method using
		 * fork(), but I didn't think it was worth it so I prefer to keep this as an infinite loop
		 * that listens to the outside world
		 *
