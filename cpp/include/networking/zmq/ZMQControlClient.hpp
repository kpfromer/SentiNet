...
private:
   ...
   * @note sock_addr can be initialized earlier with bind, but it's better to create
   * all socket->bind() calls in the same thread space, that way we're not sharing threads
   * between sockets 
   *
   * @param exit_signal checked every loop - turned on to quit
   * @param socket The socket to move
   * @note socket is not shared between threads, this is just the refernece, it is
   * bare bones constructed in the call to begin the thread
   *
   ...
   * @note this can just be a simple getter method, but be careful with race conditions
   *
  ...
  // std::ref https://en.cppreference.com/w/cpp/utility/functional/ref WILL CAUSE RACE CONDITIONS
  // THIS NEEDS TO BE HANDLED WITH EXTREME CARE!!!!!!!!!
  // static void periodic_publish_thread(const std::string sock_addr,        // Even if the thread wanted to, it can't change this
  //                              std::future<void> exit_signal,             // Can be changed from outside
  //                              std::unique_ptr<::zmq::socket_t> socket,   // A unique pointer, so once this thread is called,
  //                                                                         // The thread has complete control over it
  //                              std::string &topic,                        // Important! This can be changed, and it's an address
  //                              std::function<std::string &(void)>&,       // This can also be changed
  //                              const std::chrono::microseconds period);   // Cannot be changed


  // I want to be smarter about this, passing params by std::ref, but for now, I'm going to allow all threads to copy
  ...
  /**
   * @brief Thread That requests data from a server asynchronously at a (most likely large) period
   * @note This should be used lightly, requests are more purposful and shouldn't be automated too much
   *
   ...
   * @param callback What to do with the recieved data from the server //TODO build an abstract system that logs strings and requests that is seperate from logger
...
private:
  ...
  // inline int get_id(const std::string& descriptor, const std::string& reference);
  //
  ...
  template<typename T>
  inline socket_thread_space& create_socket(int type, T& map, const std::string& sock_addr) {
    auto socket_thread = std::make_unique<socket_thread_space>();
    map.emplace(sock_addr, std::move(socket_thread));
    map[sock_addr]->socket =
      std::make_unique<::zmq::socket_t>(context, type);
