...
/**
 * @brief I think for the future I want to make this more complex. So an io interface needs to also
 * have an allocated buffer, that way, the way we handle memory is the same across all interfaces. But
 * for now, it's just a container so  that input output interfaces can be abstracted in the future
 */
