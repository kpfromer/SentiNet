 ...
 *
 *  ZMQServer uses ZMQ server socket pattern to emulate a server. It can be
 * single or multithreaded using some simple commands to make it either one.
 *
 ...
 *
 * A ZMQServer impliments all the ZMQ functionallity (multithreading, piping
 * etc.) to create an idealized server for whatever need. Similar to a
 * publisher, but it listens and responds. For this project, a ZMQServer will
