Ralphie::Ralphie(ConfigurationClient& config)
: RobotInterface(config){
  objective.angular = 0;
  objective.linear = 0;
  objective.mining = 0;
  objective.dumping = 0;
...
void Ralphie::create_peripherals(ConfigurationClient& config)
{
  ...
  std::pair<std::string, std::shared_ptr<PeripheralInterface>> temp_pair(temp->get_name(), std::move(temp));
...
void Ralphie::update(objective_type temp)
{
  objective = temp;
}
