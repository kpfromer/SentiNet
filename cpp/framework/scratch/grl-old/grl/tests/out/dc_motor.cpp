...
float DC_Motor::convert_throttle(){
  return (pwm_zero_throttle + (constrain((const float&)throttle, (const float&)-maxThrottle, (const float&)maxThrottle) * pwm_throttle_scalar));
}

DC_Motor::DC_Motor()
: OutputInterface(), maxThrottle(10), pwm_zero_throttle(10), pin(1), pwm_throttle_scalar(5), resolution(11)
{
  throttle = 0;
  failsafe = 1;
}
...
{
  std::cout<<"writing frequency on "<<pin<<" freq "<<pwm_zero_throttle<<std::endl;
...
void DC_Motor::update(int throttle_)
{
  throttle = throttle_;
}
...
void DC_Motor::motor_write()
{
  if(failsafe)
  {
    std::cout<<"writig to"<<pin<<"throttle"<<pwm_zero_throttle<<std::endl;
  } else 
  {
    std::cout<<"writing to"<<pin<<"thottle "<<convert_throttle()<<std::endl;
  }
}
