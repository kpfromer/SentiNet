...
// Visual Studio versions below 2015 do not support sprintf properly. This is a
...
static void s_version_assert(int want_major, int want_minor) {
  ...
  if (major < want_major || (major == want_major && minor < want_minor)) {
    std::cout << "Current 0MQ version is " << major << "." << minor
              << std::endl;
    std::cout << "Application needs at least " << want_major << "."
              << want_minor << " - cannot continue" << std::endl;
    exit(EXIT_FAILURE);
  }
}
