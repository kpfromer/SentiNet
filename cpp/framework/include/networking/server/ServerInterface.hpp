...
class ServerInterface {
 public:
   ...
   * worker threads that all listen to the primary thread. Note that worker
   * threads are initialized on the stack in the method multi_threaded_listener.
   *
   ...
   *
   * @param context A number that may or may not need to be passed through the
   * method.
   *
   ...
   *
   * This is very much up to the implimentation. This can be infinitely looped
   * or not. ZMQ for example has ports tied to processes. I tried splitting this
   * method using fork(), but I didn't think it was worth it so I prefer to keep
   * this as an infinite loop that listens to the outside world
   *
