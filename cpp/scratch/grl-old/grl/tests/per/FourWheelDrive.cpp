...
void FourWheelDrive::create_outputs(ConfigurationClient& config,
                                    unsigned int id) {
  for (int i = 0; i < 4; i++) {
    dc_motor* temp = new dc_motor(config, id, i);
    std::pair<std::string, std::shared_ptr<OutputInterface>> temp_pair(
        temp->get_name(), std::move(temp));
    outputs.insert(temp_pair);
  }
}

void FourWheelDrive::create_inputs(ConfigurationClient&, std::string) {
  return;
}
